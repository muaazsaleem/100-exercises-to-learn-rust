    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let h1 = tokio::spawn(reply_to(first, Arc::clone(&reply)));
    let h2 = tokio::spawn(reply_to(second, reply));

    h1.await.unwrap().unwrap();
    h2.await.unwrap().unwrap();
}

async fn reply_to<T>(listener: TcpListener, reply: Arc<T>) -> Result<(), anyhow::Error>
where
    // `T` cannot be cloned. How do you share it between the two server tasks?
    T: Display + Send + Sync + 'static,
{
    loop {
        ...
        socket.write_all(reply.to_string().as_bytes()).await?;
    }
}
