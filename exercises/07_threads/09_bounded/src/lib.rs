...
use std::sync::mpsc::{Receiver, RecvError, SendError, Sender, SyncSender, TrySendError};
...
impl TicketStoreClient {
    pub fn insert(&self, draft: TicketDraft) -> Result<TicketId, RecvError> {
        ...
        self.sender.send(c).expect("Failed to send insert");
    ...
    pub fn get(&self, id: TicketId) -> Result<Option<Ticket>, RecvError> {
        ...
        self.sender.send(c).expect("Failed to send get");
        response_receiver.recv()
    }
}
    ...
    loop {
        match receiver.recv() {
            ...
            Ok(Command::Get {
                ...
                response_channel.send(ticket.cloned()).ok();
            }
            Err(_) => {
